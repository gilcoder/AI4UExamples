...
def make_inference_network(obs_shape, n_actions, debug=False, extra_inputs_shape=None):
	...
	'''first_ConvLSTM = tf.keras.layers.ConvLSTM2D(filters=16, kernel_size=(3,3), recurrent_activation='hard_sigmoid', activation='tanh', padding='same', name="conv1", return_sequences=True)(normalized_obs)
	first_BatchNormalization = tf.keras.layers.BatchNormalization(name="batch1")(first_ConvLSTM)
	first_Pooling = tf.keras.layers.MaxPooling3D(pool_size=(1, 2, 2), padding='same', data_format='channels_first', name="pooling1")(first_BatchNormalization)
	second_ConvLSTM = tf.keras.layers.ConvLSTM2D(filters=10, kernel_size=(3, 3), data_format='channels_first', padding='same', name="name2", return_sequences=True)(first_Pooling)
	second_BatchNormalization = tf.keras.layers.BatchNormalization(name="batch2")(second_ConvLSTM)
	second_Pooling = tf.keras.layers.MaxPooling3D(pool_size=(1, 3, 3), padding='same', data_format='channels_first', name="pooling2")(second_BatchNormalization)'''
	...
	rnn1 = tf.keras.layers.RepeatVector(32, name='rnn1')(expanded_features)
	rnn2 = tf.keras.layers.LSTM(64, return_sequences=True, name='rnn2')(rnn1)
	rnn3 = tf.keras.layers.LSTM(64, return_sequences=False, name='rnn3')(rnn2)

	hidden = tf.keras.layers.Dense(512, activation='relu', name='hidden')(rnn3)
	...
	layers = [conv1, conv2, phidden, phidden2, flattened, expanded_features, rnn2, rnn3, hidden]
...
def make_env_def(id=0):
		...
		environment_definitions['verbose'] = True
