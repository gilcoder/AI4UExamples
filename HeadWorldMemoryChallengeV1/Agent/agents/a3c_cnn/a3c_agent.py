...
def make_inference_network(obs_shape, n_actions, debug=False, extra_inputs_shape=None):
	...
	normalized_obs = tf.keras.layers.Lambda(lambda x : x/26.0)(observations)

	# Numerical arguments are filters, kernel_size, strides
	conv1 = tf.keras.layers.Conv2D(1, (1,1), (1,1), activation='relu', name='conv1')(normalized_obs)
	if debug:
	...
	#conv2 = tf.keras.layers.Conv2D(16, (3,3), (3,3), activation='relu', name='conv2')(conv1)

	hp1 = tf.keras.layers.Dense(30, activation='relu', name='phidden')(proprioceptions[:, 0:ARRAY_SIZE])
	hp2 = tf.keras.layers.Dense(30, activation='relu', name='phidden2')(hp1)

	flattened = tf.keras.layers.Flatten()(conv1)
	expanded_features = tf.keras.layers.Concatenate()([flattened, hp2])

	hidden1 = tf.keras.layers.Dense(64, activation='relu', name='hidden1')(expanded_features)
	...
	layers = [conv1, hp1, hp2, flattened, expanded_features, hidden1, hidden2]
