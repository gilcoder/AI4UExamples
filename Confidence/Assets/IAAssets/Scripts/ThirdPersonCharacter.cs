...
namespace UnityStandardAssets.Characters.ThirdPerson
{
	...
	public class ThirdPersonCharacter : MonoBehaviour
	{
        ...
        public void Move(Vector3 move, bool crouch, bool jump, float m_turnHorizontal=0.0f, float turnVertical=0.0f, bool pushing=false, float xspeed=0.0f, float yspeed=0.0f, bool pickup=false)
		{
			...
			if (yspeed != 0){
				if (yspeed <= 1.0){
                    playerSceneLogic.SetEnergy(playerSceneLogic.GetEnergy() - 1);
                } else {
                    playerSceneLogic.SetEnergy(playerSceneLogic.GetEnergy() - 3);
                }
            }
            ...
            if (pushing)
                playerSceneLogic.SetEnergy(playerSceneLogic.GetEnergy() - 2);

            if (System.Math.Abs(m_turnHorizontal-lastTurnHor) > 10)
                playerSceneLogic.SetEnergy(playerSceneLogic.GetEnergy() - 1);

            if (System.Math.Abs(m_turnVertical-lastTurnVer) > 10)
                playerSceneLogic.SetEnergy(playerSceneLogic.GetEnergy() - 1);
			*/
        ...
        void ApplyExtraTurnRotation()
        {
            ...
            if (Camera.main != null)
            {
                ...
                Camera.main.transform.Rotate(d, 0, 0);
                float ang = Camera.main.transform.rotation.eulerAngles.x;
                // Debug.Log(ang);
                if (ang > 300 && ang < 320)
                {
                    Camera.main.transform.Rotate(-d, 0, 0);
